...
namespace Booter
{
    ...
    public class InstalledVersionManager
    {
        ...
        public static string GetExeFileName()
        {
            ...
            {
                fn = GetReleaseVersionExeFileNameForTest();
            }
            if (string.IsNullOrEmpty(fn))
            {
        ...
        {
            var fn = StartUpPathOfExe + "Release\\Bot.exe";
            if (!File.Exists(fn))
            {
                fn = null;
            }
            ...
            try
            {
                ...
                using (var reader = new StreamReader(path))
                {
                    ...
                    if (fullVersion.StartsWith("v"))
                    {
                        ...
                        exeFn = GetExistPathByVersion(version) + "\\Bot.exe";
                    }
                }
        ...
        private static List<InstalledVersion> GetAllInstalledVersionAndSortByVersionDesc()
        {
            ...
            var dirts = Directory.GetDirectories(StartUpPathOfExe, "v*");
        ...
        private static string GetExistPathByVersion(int version)
        {
            ...
            {
                var installedVersion = allInstalledVersions.SingleOrDefault(k => k.Version == version);
