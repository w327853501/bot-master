...
using Bot.Common.Db;
using Bot.AssistWindow.Widget.Right.CouponNs;
	...
	public partial class RightPanel : UserControl, IWakable
    {
		...
		public void Init(WndAssist wnd)
		{
			if (_wndDontUse == null)
			{
				...
				var tabCsv = Params.Panel.GetRightPanelCompOrderCsv(Wnd.Desk.Seller);
		...
		public void UpdateShortcutUI()
		{
			DispatcherEx.xInvoke(()=>{
                try
			    {
				    var tabItem = GetTabItem(TabTypeEnum.ShortCut);
		...
		public void ReShowAfterChangePanelOption()
		{
			...
			foreach (var tabName in tabs)
			{
				var tabVisible = Params.Panel.GetPanelOptionVisible(Wnd.Desk.Seller, tabName);
				var tabType = GetTabType(tabName);
                var tabItem = CreateTabItem(tabType);
                if (tabItem != null)
                {
		...
		private TabItem TabCoupon()
		{
			return new TabItem
			{
				Header = "优惠券",
				Content = new CtlCouponPanel(Wnd.Desk)
			};
		}
        ...
        public void SelectGoodsTabItem()
		{
			var selectedItem = GetSelectedTabItem(TabTypeEnum.Goods);
			tabControl.SelectedItem = selectedItem;
		}

		private TabItem GetSelectedTabItem(TabTypeEnum tabType)
		{
			TabItem tabItem = null;
        ...
        {
            DbSyner.EvSynFinished -= DbSyner_EvSynFinished;
            DispatcherEx.xInvoke(()=>btnSyn.IsEnabled = true);
            if (e.IsOk)
            {
