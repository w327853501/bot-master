...
namespace Bot.AssistWindow
{
    public partial class WndAssist : Window
    {
        ...
        public WndAssist(ChatDesk desk)
        {
            ...
            if (!DesignerProperties.GetIsInDesignMode(this))
            {
                try
                {
                    ...
                    FontSize = Params.Other.FontSize;
                    Desk = desk;
                    this.xHideToAltTab();
                    Left = 0.0;
                    Top = 0.0;
        ...
        private void Desk_EvGetForeground(object sender, ChatDeskEventArgs e)
        {
            ...
            {
                ShowAssist();
                if (_foregroundTrackDelayCaller != null)
                {
                    _foregroundTrackDelayCaller.CallAfterDelay();
                }
            });
        }
        ...
        private void Desk_EvMaximize(object sender, ChatDeskEventArgs e)
        {
            ...
            DispatcherEx.xInvoke(() =>
            {
                double maxW = SystemParameters.WorkArea.Width - WaParams.GetRightPanelWidth(Desk.Seller) - (double)12f;
                int w = (int)(maxW / ToLogicalRatio.XRatio);
                double maxH = SystemParameters.WorkArea.Height - WaParams.GetBottomPanelHeight(Desk.Seller) - (double)12f;
        ...
        {
            var wndAssist = new WndAssist(desk);
