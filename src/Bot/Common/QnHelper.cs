...
namespace Bot.Common
{
    public class QnHelper
    {
        public class Detected
        {
            ...
            public static string[] GetLatest5SecDetectedNicksNotNull()
			{
				...
				try
				{
					...
					nicks = (set.ToArray() ?? new string[0]);
				}
				catch (Exception e)
				{
					Log.Exception(e);
        ...
        public class Auth
		{
			...
			static Auth()
			{
				_key = HybridKey.SuperSubAccount.ToString();
				ShortcutCantEditTipCount = 0;
            }

            public static bool IsSuperAccount(string seller)
			{
				var mainPart = TbNickHelper.GetMainPart(seller);
				return mainPart == seller || GetSuperAccounts(mainPart).Contains(seller);
			}
			...
			public static HashSet<string> GetSuperAccounts(string mainNick)
			{
                ...
                var superAccs = HashSetEx.Create(subAccs.Select(k => mainNick+":"+k));
            ...
            public static bool IsActiveSuperAccount(string seller, string desc = null)
			{
				...
				{
					var superAccounts = GetSuperAccounts(mainPart);
			...
			{
				return IsActiveSuperAccount(seller, showDesc ? "删除【全部】顾客便签" : null);
			}
            ...
            public static bool CanEditShortCut(string seller, string desc = "编辑话术")
			{
				...
				else if (ShortcutCantEditTipCount < 3)
				{
					if (!(rt = IsActiveSuperAccount(seller, desc)))
					{
						ShortcutCantEditTipCount++;
					}
				}
				else if (!(rt = IsActiveSuperAccount(seller, null)))
				{
					Util.Beep();
				}
				return rt;
			...
			{
				return Params.Auth.GetIsAllAccountEditKnowledge(seller) || IsActiveSuperAccount(seller, desc);
			}

            public static bool CanExportKnowledge(string seller)
			{
				return CanEditKnowledge(seller, "导出宝贝知识");
			}

            public static bool CanEditRobotRule(string seller)
			{
				return Params.Auth.GetIsAllAccountEditRobot(seller) || IsActiveSuperAccount(seller, "编辑机器人规则");
			}
        ...
        public static bool IsGreaterV9_03_13N()
        {
            return !IsLessQnVersion(9, 03, 13);
        }
        public static bool IsGreaterV9_02_00N()
        {
            return !IsLessQnVersion(9, 02, 00);
        }
        public static bool IsGreaterV9_01_00N()
        {
            return !IsLessQnVersion(9, 01, 00);
        }
        public static bool IsGreaterV9_00_02N()
        {
            return !IsLessQnVersion(9, 00, 02);
        }

        public static bool IsGreaterV9_00_00N()
        {
            return !IsLessQnVersion(9, 00, 00);
        }
