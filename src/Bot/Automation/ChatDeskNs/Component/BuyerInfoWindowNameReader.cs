...
namespace Bot.Automation.ChatDeskNs.Component
{
    public class BuyerInfoWindowNameReader : Disposable
    {
        ...
        {
            string guestNameImageB64 = GetImageB64Str(true);
        ...
        private string GetImageB64Str(bool noCache)
        {
        ...
        private string GetBuyerNameUseCache()
        {
            ...
            if (_desk.IsBoundaryVisable(buyerNameRegion))
            {
                var buyerImageB64Str = GetImageB64Str(true);
                ...
                else
                {
                    ...
                    {
                        string buyerImageB64StrNoCache = GetImageB64Str(false);
        ...
        public string GetBuyerNameFromBuyerInfoWindow()
        {
            ...
            if (buyerInfoHwnd > 0)
            {
                WinApi.HideWindow(buyerInfoHwnd);
    ...
    //    }
        ...
        private string GetBuyerNameFromBuyerInfoWindow(int pHwnd)
        {
            ...
            var buyerInfoWndText = WinApi.GetText(pHwnd);
            if (buyerInfoWndText != null)
            {
                buyerInfoWndText = buyerInfoWndText.Trim();
                if (buyerInfoWndText.EndsWith("的资料"))
                {
                    buyerName = buyerInfoWndText.Substring(0, buyerInfoWndText.Length - 3);
                }
            }

            #region[千牛7.39.20之前版本的获取方法]
            //for (int i = 0; i < 50; i++)
            //{
