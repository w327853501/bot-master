...
namespace Bot.Automation.ChatDeskNs.Component
{
    public class TaskWindowBuyerNameReader : Disposable
    {
        ...
        public TaskWindowBuyerNameReader(ChatDesk desk)
        {
            _isTest = false;
        ...
        private void Loop()
        {
            try
            {
                if (IsFrozenDetected || !_desk.IsChatRecordChromeOk || (string.IsNullOrEmpty(_desk.Buyer) && _desk.GetIsSingleChatCloseButtonEnable(true)))
                {
                    BuyerChanged();
                }
                if (!IsFrozenDetected && _desk.IsChatRecordChromeOk && _desk.IsVisible && _desk.GetIsSingleChatCloseButtonEnable(true) && _desk.GetBuyerRegionVisibleUseCache(true) && _desk.IsForegroundOrVisibleMoreThanHalf(true))
                {
                    DetectFrozen();
                }
            }
        ...
        private void DetectFrozen()
        {
            try
            {
                ...
                var imageB64StrCache = _frozenCache[buyer];
        ...
        private void BuyerChanged()
        {
                ...
                && _desk.IsForegroundOrVisibleMoreThanHalf(true) && Wnd != null)
            {
                ...
                if (guestName != _desk.Buyer)
                {
                    ...
                    {
                        EvBuyerSwitched(this, new BuyerSwitchedEventArgs() { PreBuyer = _desk.Buyer,CurBuyer = guestName,FromTaskWindow = true });
                    }
                }
            }
        }
        ...
        {
            var guestNameImageB64 = GetImageB64StrNoCache(true);
        ...
        {
            var buyerNameRegion = _desk.Automator.GetBuyerNameRegion();
            var tmpRect = new Rectangle(buyerNameRegion.Left, buyerNameRegion.Top - 10, buyerNameRegion.Width, 40);
            SaveDrawBitmap(tmpRect);
        }

        private string GetImageB64Str(Rectangle buyerNameRegion)
        {
            var newB64 = string.Empty;
            try
            {
                var buyerNameRegionImage = DrawToBitmap(buyerNameRegion);
