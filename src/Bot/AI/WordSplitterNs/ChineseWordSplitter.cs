...
namespace Bot.AI.WordSplitterNs
{
    public class ChineseWordSplitter
    {
        ...
        {
            var innerSplitInfos = new List<SplitInfo>();
        ...
        {
            var overlappedSplitInfos = new List<List<SplitInfo>>();
            if (fromidx >= s.Length) return overlappedSplitInfos;
            var sInfos = SplitWord(s, fromidx);
            if (sInfos.Count == 0)
            {
            ...
            else
            {
                foreach (var si in sInfos)
                {
                    ...
                    var ovlpdSplitInfos = SplitOverlappedWord(s, si.NextIndex);
        ...
        {
            var list = new List<List<SplitInfo>>();
            int idx = 0;
            for (int i = fromidx; i <= toidx; i = idx + 1)
            {
                ...
                var overspinfos = SplitOverlappedWord(s, idx + wlen);
                list.AddRange(MergerSplitInfos(splitInfos, overspinfos));
            }
            return list;
        }
        ...
        {
            var mergerSplitInfos = new List<List<SplitInfo>>();
