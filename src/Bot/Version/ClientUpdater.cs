    ...
    public class ClientUpdater
    {
        ...
        private async static void ManualUpdate()
        {
            try
            {
                ...
                {
                    MsgBox.ShowDialog(string.Format("正在下载升级文件，版本=【{0}】，不要关闭软件，大概需要3分钟。", ShareUtil.ConvertVersionToString(updtEt.PatchVersion)));
                    Update(updtEt);
                }
            }
            catch (Exception ex)
        ...
        private static void Update(UpdateDownloadEntity updtEt)
		{
			if (!_isUpdating)
			{
				...
				{
					Util.WaitFor(() => {
						return Process.GetProcessesByName("Bot").Length < 1;
					}, 2000, 100);
					Log.Info(string.Format("开始升级，补丁={0}", Util.SerializeWithTypeName(updtEt)));
					newVerDir = PathEx.ParentOfExePath + ShareUtil.ConvertVersionToString(updtEt.PatchVersion);
					...
					else
					{
						...
						if (MsgBox.ShowDialogEx(msg, "提示", null, null, null))
						{
							Reboot();
						}
					}
