    ...
    public class MyWebSocketServer
    {
        ...
        public ushort Port
        {
            get
            {
                if (_port <= 0)
                {
                    _port = (ushort)FreePort.FindNextAvailableTCPPort(31148);
                }
                return _port;
            }
        }
        ...
        public void Start()
        {
            try
            {
                ...
                {
                    if (OnRecieveMessage != null) OnRecieveMessage(this, new RecieveMsgEventArgs("", value));
                };
                webSocket.SessionClosed += (session, value) =>
                {
                    Console.Write(value);
                };
                webSocket.Setup("127.0.0.1", Port);//设置端口
                webSocket.Start();
            }
            catch (Exception ex)
            {
    ...
    public class MyHttpServer
    {
        ...
        public ushort HttpPort
        {
            get
            {
                if (_port <= 0)
                {
                    _port = (ushort)FreePort.FindNextAvailableTCPPort(31138);
                }
                return _port;
            }
        }
