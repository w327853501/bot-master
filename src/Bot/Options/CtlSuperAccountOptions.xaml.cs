...
namespace Bot.Options
{
	public partial class CtlSuperAccountOptions : UserControl, IOptions
    {
		...
		{
            var superAccounts = tboxNicks.Text.Replace(' ', ',').Replace('\u3000', ',');
            Params.Auth.SetSuperAccounts(_sellerMain, superAccounts);
			Params.Auth.SetIsAllAccountEditKnowledge(seller, rbtKnAll.IsChecked ?? false);
			Params.Auth.SetIsAllAccountEditRobot(seller, rbtRobotAll.IsChecked ?? false);
			Params.Auth.SetIsAllAccountEditShortCut(seller, rbtScAll.IsChecked ?? false);
		}
	}
}
